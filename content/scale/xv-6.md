...
스트리밍 집계 파이프라인의 "탑 K" 계산도 Count-Min Sketch 위에 얹히는 경우가 많다 — 정확한 순위가 아니라 "대략 가장 많이 등장한 항목 K개"면 충분한 상황에서, 스트림의 모든 항목을 정렬하지 않고도 근사치를 낼 수 있다.
...
- Count-Min Sketch는 카운터 격자에 $d$개의 해시로 값을 올리고 조회 시 최솟값을 취해 빈도를 추정한다. 자주 등장하는 항목(핫키)일수록 여러 칸의 값이 동시에 크므로 최솟값도 정확해진다.
