...
- 정확한 카디널리티는 원소 전체를 기억해야 하므로 공간이 $O(n)$이다. HyperLogLog는 해시값의 "처음 나오는 1비트 위치"를 레지스터 여러 개에 나눠 최댓값으로 저장해 공간을 $O(m)$(미리 정한 상수)으로 줄인다.
- 실측 상대오차(1.59%)가 이론값 $1.04/\sqrt{m}$(3.25%) 이내에 들어온다 — 공식이 실제 동작을 설명한다.
- **레지스터는 원소를 기억하지 않고 "가장 운이 좋았던 값"만 남긴다.** 그래서 중복 추가는 무시되고, 서버 간 병합이 레지스터별 최댓값 취하기로 간단히 된다.
- Count-Min Sketch는 카운터 격자에 $d$개의 해시로 값을 올리고 조회 시 최솟값을 취해 빈도를 추정한다. 자주 등장하는 항목일수록 정확하다.
- **과대추정만 있고 과소추정은 없다.** [XV-4](#/xv-4) 블룸 필터의 "거짓양성만 있고 거짓음성은 없다"와 같은 형태의 단방향 오차이고, 실측(과소추정 0건)이 이 성질을 그대로 보여준다.
- 대표 용례는 유니크 방문자 수(HyperLogLog)와 핫키 검출(Count-Min Sketch) — 둘 다 "정확한 개별 값보다 전체 규모나 상대적 크기가 중요한" 지표다.
- [XV-4 블룸 필터](#/xv-4)·[XV-5 스킵 리스트](#/xv-5)·이 챕터는 같은 계보다. **정확성을 메모리와 교환하되, 어느 방향으로 틀릴지는 설계로 통제한다.**

**다음 절**: [XV-7 머클 트리와 해시 체인](#/xv-7) — 이번엔 근사가 아니라 정확성을 지키면서도, 전체를 비교하지 않고 차이만 찾아내는 방법을 본다.
